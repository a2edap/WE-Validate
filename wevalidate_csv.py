    ...
    if conf['analysis'][0] == 'base':
        ...
        for ind, c in enumerate(comp):
            ...
            cal_print_metrics_csv.run(
                combine_df, metrics, results, ind, c, conf, base, aggregations
                )
        ...
        for ind, c in enumerate(comp):
            ...
            for df in (magnitude, ramprate, duration):

                cal_print_metrics_csv.run(
                   df, metrics, c, conf, base
                    )
                ...
                if all_lev_df.empty:
                    all_lev_df = pd.concat([all_lev_df, df], axis=1)
                else:
                    all_lev_df = pd.concat([all_lev_df, df], axis=1)
